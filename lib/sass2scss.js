...
var unquotedImportRegex = /^@import\s([^\"])(.*)([^\"])$/;
...
var quoteImportStatements = function(line) {
  var match = unquotedImportRegex.exec(line);
  if (match != null) {
    var m = ""
  ...
  if (input) {
    ...
    input.split('\n').forEach(function(line){
      ...
      if(! line.match(/^\s*(\/\*.*\*\/.*)?(\/{2}.*)?$/) ){
        ...
        line = quoteImportStatements(line);
